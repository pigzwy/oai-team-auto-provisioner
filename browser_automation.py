...
from s2a_service import s2a_generate_auth_url
...
def perform_codex_authorization(page, email: str, password: str) -> dict:
    ...
    # S2A 模式: 直接返回 code 和 session_id，由 run.py 调用 s2a_create_account_from_oauth
    if AUTH_PROVIDER == "s2a":
        log.success("获取授权码成功 (S2A)")
...
def perform_codex_authorization_with_otp(page, email: str) -> dict:
    ...
    # S2A 模式: 直接返回 code 和 session_id
    if AUTH_PROVIDER == "s2a":
        log.success("获取授权码成功 (S2A/OTP)")
        return {"code": code, "session_id": session_id}
        ...
        for attempt in ctx.attempts():
            try:
                ...
                if AUTH_PROVIDER == "cpa":
                    ...
                    # S2A 模式: perform_codex_authorization 返回 code 和 session_id
                    ...
                    if (
                        ...
                        from s2a_service import s2a_create_account_from_oauth
                        ...
                        return {
                            ...
                            "crs_id": f"S2A-{s2a_result.get('id')}"
                            if s2a_result
                            else None,
                        }
                    else:
                        if attempt < ctx.max_retries - 1:
                            log.warning("S2A 授权数据获取失败，准备重试...")
                            continue
                        return {
                            "success": False,
                            "token": token,
